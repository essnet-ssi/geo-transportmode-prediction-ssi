...
def feature_preparation_dt(events, columns_to_factorize):
    """
    Prepares features in events data to be processed in a decision tree. For other models, other processing steps may be required.

    :param events: Dataframe with events
    :param columns_to_factorize: List of column names that should be factorised
    :return: Dataframe with events, with features prepared for use by a decision tree
    """
...
def get_train_test(events, test_ratio, columns_to_factorize, columns_to_exclude, random_seed):
    ...
    events = feature_preparation_dt(events, columns_to_factorize)
    
    ## Split train-test set
    # Prepare columns to be removed (so only the columns present will actually be removed):
    columns_to_exclude_selection = [c for c in columns_to_exclude if c in events.columns]
    ...
    ## Prepare train and testset variables
    train_data = train_data.drop(columns_to_exclude_selection, axis=1)
    test_data = test_data.drop(columns_to_exclude_selection, axis=1)
...
def train_decision_tree_gs(train_X, train_y, param_grid, random_seed):
    ...
    :param random_seed: To control the random state of the model fit
...
def dt_feature_importance(trained_model):
    ...
    :param trained_model: Object of trained decision tree model
...
def visualize_dt(trained_model, save_loc = None):
    ...
    :param trained_model: Object of trained decision tree model
    :param save_loc: Location where visualisation will be saved

    no return: plot is displayed
    """
...
def evaluate_dt(trained_model, test_X, test_y_true):
    """
    Returns various evaluation metrics of the trained model for a given test set 

    :param trained_model: Object of trained model
    :param test_X: Features of test set
    :param test_y_true: True labels of test est
    ...
    classes_1 = test_y_true.unique().tolist()  # may contain unseen labels
    classes_2 = trained_model.classes_.tolist()  # trained labeks
    label_names = sorted(set(classes_1 + classes_2))  # all unique labels
    ...
    labels_list = (test_y_pred + test_y_true).unique()
    class_report = metrics.classification_report(y_true=test_y_true, y_pred=test_y_pred, 
...
def dt_to_text(trained_model):
    ...
    no return: print directly
    """
    # Prepare objects to loop through 
